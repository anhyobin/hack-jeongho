...
>
> **A nickname that ever posted an inflated score is burned for good — use a never-used one for anything you want to look clean.** Every board entry carries `rep` (0 = clean, 2 = suspect, 3 = abuse-suspected) plus a Korean `rep_why`. The 08-15 A/B settles what the mark follows: `정호야호 777` (honest, 719 rows actually crossed, `verified: true`) got `rep 2` while `야호정호 602` — same IP, same autopilot build, same input profile, 10 hours earlier — is still `rep 0`. The difference is that `정호야호` had posted an inflated `3000 / rows 3000` on 08-13. So the mark follows the **nickname, not the source** (`rep_why` says "곳"/place, which misleads), it **survives deletion of the offending entry**, and it applies to submissions that **succeeded** at the time — the server re-judged old rows under the new model (`docs/leaderboard-api.md` §9.4).
>
