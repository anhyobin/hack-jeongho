...
**Current records (2026-08-17):** the board's top three are all ours — `정호돌격 700 / 654 rows / stage 32`, `야호정호 600 / 556`, `정호야호 500 / 466`, all `verified: true`, `rep 0`, no duplicates. Deepest grant seen is **chunk 27**, so the old "chunk 26 / 25-grant cap" guess in `tools/local_proxy.py` is dead. `핵정호` is deliberately unregistered, reserved for 1000.

**1000 points is out of reach with this architecture, and the arithmetic is written down** (`docs/autopilot.md` §13.5): by rows it needs ~935 rows against a measured median frontier of 470 and a best of 654; by bonus it needs ~175 near-misses, and near-miss farming is tick-expensive while pacing plus `TOKEN_STALE_SEC = 600` caps a run at roughly 36,000 ticks (654 rows already costs 14,275). `bfarm=1` (hold position so flagged goranis pay out) works and lifts recovery to 100%; `bfarm=2` (seek flags) does not — the safe band is only 62-84 px ≈ 4-7 ticks against a 8-9 tick hop cadence. Default is `bfarm=0`.
