...
>
> **When chunks start getting refused, stop — do not rotate tokens.** Check the reached row first (`docs/leaderboard-api.md` §10.3.1: a refused chunk *usually* means the bot did not actually get there). But if the reached row is fine, the variable is probably not in your request at all: on 08-15 the afternoon's six tokens were refused at chunk 3/66 rows, 8/190, 9/215 (twice, same row), 10/240 and 18/440 — **every one of those exact (chunk, reached row) pairs had been granted that morning** (§11.3.1). Depth, `ticks`, chunk index, token age and per-token grant count are all ruled out by that table. Meanwhile a third party registered normally in the same window, so it is not an outage. Rotating tokens cost 229 requests and zero registrations; the earlier "throw the token away" rule came from a single 11:03→11:11 observation and did not survive. **Cool off for hours, then retry exactly once to judge.**
>
...
**Probe the gate depth before committing to a live run — `docs/autopilot.md` §12.5.** The chunk gate closes on us for reasons we have not pinned down (§12.4: grant depth fell 18 → 2 over 08-15 while our refusals piled up to 284 against 78 grants; five hours of cooling and a fresh deploy both changed nothing). With the fail-fast constants (`SEARCH_WAIT_ABORT 5.0 < SEARCH_REQ_INTERVAL 6.0`, `SEARCH_CHUNK_FAIL_MAX 1`) that question is answered in **41 seconds, 3 requests, 1 refusal** — and if the gate *is* open the same run continues straight to the target, so the probe is free. Depth 2-4 means stop; 12+ means go. Retrying a refused chunk has never once worked (0 for 9), so never raise `SEARCH_CHUNK_FAIL_MAX` back up: refusals appear to be what closes the gate, and they are self-inflicted.
