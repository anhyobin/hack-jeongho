...
### 5. 파이썬 포팅 (★ 이 항목은 틀렸다 — `wt/rng`가 뚫었다)

> **정정:** 아래 "다시 시도하지 마라"는 잘못된 판단이었다. `wt/rng`가 원인을 찾아냈다 —
> 시딩만이 아니라 **`randf()`가 `rand()`를 두 번 먹는 것**이 진짜 원인이었고, 아래
> "배제한 것" 목록은 **틀린 `randf`로 판정한 것**이라 애초에 통과할 수 없었다.
> 지금은 `tools/sim.py`가 실제 주행을 사망 틱까지 재현한다 → `docs/autopilot.md` §9.
> 원문은 판단 오류의 기록으로 남긴다.
