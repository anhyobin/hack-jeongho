...
var tick_count := 0
var hop_end_tick := -1
var pending_input := Vector2i.ZERO
var input_trace: Array = []
var replay_mode := false
var replay_inputs: Array = []
var replay_idx := 0

static func _dircode(d: Vector2i) -> int:
	if d.y > 0: return 0
	if d.y < 0: return 1
	if d.x < 0: return 2
	return 3
...
func _sim_tick(dt: float) -> void:
	...
	var py := world.position.y + player.rest_pos().y
	if py > 1000.0:
		kill_player("scroll")
		return
...
func _update_shake(dt: float) -> void:
	if shake_t > 0.0:
		...
		var jitter := (vrng if vrng != null else rng).randf_range(-1.0, 1.0)
		world.position.x = jitter * 7.0 * (shake_t / 0.35)
		if shake_t <= 0.0:
...
func try_move(dir: Vector2i) -> void:
	if state != "play" or player.dead or replay_mode:
		return
	pending_input = dir
...
func _consume_input() -> void:
	if player.hopping or player.dead or state != "play":
		return
	var dir := _next_input()
	if dir != Vector2i.ZERO:
		_apply_move(dir)

func _apply_move(dir: Vector2i) -> void:
	...
	if not replay_mode:
		input_trace.append([tick_count, _dircode(dir)])
		main.sfx.play("hop", -6.0, 1.0, 0.06)
	hop_end_tick = tick_count + HOP_TICKS
	player.hop(dir, to_row, to_x)
