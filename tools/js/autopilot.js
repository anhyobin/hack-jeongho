...
// 실측 보정값 (2026-08-14, 캔버스 700x1000 / 게임 640x960 / scale 1.0417)
//   세로 스캔으로 확정: 플레이어 중심 화면 y = 584, 행 높이 = 66.67px
//   row+k 중심 = 584 - 66.67*k   (k=3이 도로 밴드 358~424와 일치해 검증됨)
//   색: 풀 rgb(126,200,80) / 도로 rgb(74,74,82) / 철로 rgb(154,138,114) / 페키 rgb(247,152,29)
//   앞선 버전은 플레이어를 y 425로 가정해 2.5행 어긋난 곳을 보고 있었다.
//
// 실측한 실패 모드 3가지 (각각 한 번의 실행으로 확인):
//   1) alive()를 단일 라인으로 보면 홉 중 공중에 있는 플레이어를 놓쳐 오탐 종료한다.
//      -> 세로 밴드(y 584+40 ~ 584-90)를 훑어야 한다. 수정 후 12행 전진 확인.
//   2) 풀 행의 "막힘" 판정을 밝기로 하면 거의 항상 막힘으로 나온다(풀 하이라이트·장식).
//      -> 나무/바위/부시의 실제 색으로 판정해야 한다. 현재 4홉/124대기로 사실상 정지.
//   3) 대기가 길면 강제 스크롤에 죽는다(22.7초에 사망). 상한이 0.62행/초이므로
//      "N초 이상 못 움직였으면 위험을 감수하고 전진" 같은 하한 규칙이 필요하다.
//
// 남은 작업: 위 2)·3) 해결, 두 프레임 비교로 차량 속도 추정, 좌우 회피.
// 이 봇이 버는 행이 곧 점수다 — 서버가 trace를 재현해 rows를 검증하므로
// 등록 가능한 점수 = 실제로 넘은 행 수 * 2 + 40 (docs/leaderboard-api.md §7).
